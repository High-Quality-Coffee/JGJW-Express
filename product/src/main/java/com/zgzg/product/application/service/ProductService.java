...
public class ProductService {
	...
	public ApiResponseData<String> create(ProductRequestDTO productRequestDTO) {
		...
		if (productRepository.existsProduct(productRequestDTO.getProductName(), productRequestDTO.getStoreId(),
			productRequestDTO.getHubId())) {
			return ApiResponseData.failure(Code.PRODUCT_ALREADY_EXISTS.getCode(),
				Code.PRODUCT_ALREADY_EXISTS.getMessage());
		}
	...
	public ApiResponseData<String> addProduct(ProductStockRequestDTO productStockRequestDTO) {
		...
		Product product = productRepository.findById(productStockRequestDTO.getId())
			.orElseThrow(() -> new BaseException(Code.PRODUCT_NOT_EXISTS));
